...
def print_results(chests,f):

    locale.setlocale(locale.LC_ALL, '')
    for chest in chests:
        itemcount = sum(chest.items.values())
        f.write("%s at %s,%s,%s with %d items:" % \
            (chest.type,\
        ...
        for blockid,count in chest.items.items():
            if type(blockid) == int:
                f.write("   %3dx Item %d" % (count, blockid))
            else:
                f.write("   %3d x %s" % (count, blockid))
...
def main(world_folder):
    ...
    try:
        for chunk in world.iter_nbt():
            print_results(chests_per_chunk(chunk["Level"]),f)
        f.close()
